...
```
## 88.how parent an child process executes different blocks of code after fork()?
After fork(), both parent and child begin from the same point, but they execute different code blocks based on the return value of fork() —
0 for child, positive PID for parent
## 89.what adress cpu places an adress bus?
```
When the CPU wants to read or write data, it first puts the memory address (where that data is located or should be stored) onto the address bus.
The address bus then carries this address to main memory (RAM) or I/O devices.
Example:
If the CPU wants to read data from memory location 0x2000 —
→ It places 0x2000 on the address bus.
→ The control unit sends a READ signal.
→ The memory module at address 0x2000 places the requested data on the data bus.
```
## 90.when virtual adress are generated ?
```
Virtual addresses are generated by the CPU when a program is executing — specifically, during instruction execution whenever the process accesses memory
Each process in a system is given its own virtual address space.
When the CPU executes an instruction that refers to a memory location (for example, MOV A, [1000]),
→ the CPU generates a virtual address (VA) — in this case, 1000.
This virtual address is then sent to the Memory Management Unit (MMU).
The MMU translates the virtual address → physical address using the page table.
| Step | Action                                            |
| ---- | ------------------------------------------------- |
| 1    | Program executes an instruction involving memory. |
| 2    | CPU generates a **virtual address**.              |
| 3    | MMU translates it to a **physical address**.      |
| 4    | Data/instruction is fetched or stored in memory.  |
```
## 91.when physical adresss are generated from virtual adress ?
```
Physical addresses are generated at the time of address translation by the MMU (Memory Management Unit) — that is, when a virtual address is sent by the CPU to access memory.
Step-by-step (simple explanation):
CPU executes an instruction (like reading/writing a variable).
That variable has a virtual address (used by your program).
The MMU (Memory Management Unit) takes this virtual address.
Using the page table, the MMU translates it to a physical address.
The physical address is then sent on the address bus to access data from RAM.
```
## 92.what is the difference between Execl() and Execv()?
```
| Feature                      | `execl()`                                                        | `execv()`                                                             |
| ---------------------------- | ---------------------------------------------------------------- | --------------------------------------------------------------------- |
| **How arguments are passed** | As a **list of arguments** (comma-separated), ending with `NULL` | As an **array of strings (argv[])`**, terminated by `NULL`            |
| **Usage style**              | `execl(path, arg0, arg1, ..., NULL);`                            | `execv(path, argv);` where `argv` is a char* array                    |
| **When to use**              | When you know all arguments at compile time                      | When arguments are stored in an array (dynamic)                       |
| **Example**                  | `execl("/bin/ls", "ls", "-l", "/home", NULL);`                   | `char *args[] = {"ls", "-l", "/home", NULL}; execv("/bin/ls", args);` |
```
## 93.What is the difference between Execlp() and Execvp()?
```
| Feature               | `execlp()`                                        | `execvp()`                                                   |
| --------------------- | ------------------------------------------------- | ------------------------------------------------------------ |
| **Arguments passing** | Takes a **list of arguments**, ending with `NULL` | Takes an **array of strings (`argv[]`)**, ending with `NULL` |
| **Search in PATH**    | Yes, searches for executable in PATH              | Yes, searches for executable in PATH                         |
| **Usage style**       | `execlp("ls", "ls", "-l", NULL);`                 | `char *args[] = {"ls", "-l", NULL}; execvp("ls", args);`     |
| **When to use**       | When arguments are **known and fixed**            | When arguments are **dynamic or stored in an array**         |
```
